...
/*	while(1)
	{
		if(!OLED_SDA)				//ÅĞ¶ÏÊÇ·ñ½ÓÊÕµ½OLED Ó¦´ğĞÅºÅ
		{
...
/*
	@brief			ÏÔÊ¾ÖĞÎÄ
	@param			x£ºÆğÊ¼ÁĞ£»Ò»¸ö×ÖÌåÕ¼16ÁĞ
					y£ºÆğÊ¼Ò³£»Ò»¸ö×ÖÌåÕ¼Á½Ò³
					no£º×ÖÌåµÄĞòºÅ
					m:µ¥¸ö×ÖÌå´óĞ¡
					Chn[][m]:ÖĞÎÄÊı×é
	@retval			ÎŞ
...
//	    		{
//						
//	    			OLED_WR_Byte(GIF[i][j++],OLED_DATA);	//»­Í¼Æ¬µÄµã    	
//	    		}
//		}
...
{ 	
	...
	RCC_APB2PeriphClockCmd(RCC_APB2Periph_GPIOC, ENABLE);	 //Ê¹ÄÜA¶Ë¿ÚÊ±ÖÓ
	GPIO_InitStructure.GPIO_Pin = GPIO_Pin_14|GPIO_Pin_15;	 
 	GPIO_InitStructure.GPIO_Mode = GPIO_Mode_Out_PP; 		 //ÍÆÍìÊä³ö
	GPIO_InitStructure.GPIO_Speed = GPIO_Speed_50MHz;//ËÙ¶È50MHz
 	GPIO_Init(GPIOC, &GPIO_InitStructure);	  //³õÊ¼»¯GPIOD3,6
